...
export default function Button({
  ...
  if (url) {
    return (
      <a href={url} className={combinedClasses} download={download} target={target} rel={rel} {...rest}>
        {children}
      </a>
    );
  }
  ...
  return (
    <button type={type} onClick={onClick} className={combinedClasses} disabled={disabled} {...rest}>
      {children}
    </button>
  );
}
